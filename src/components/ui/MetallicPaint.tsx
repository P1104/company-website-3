...
export default function MetallicPaint({
  ...
  // Merge default params with provided params using useMemo
  const finalParams = useMemo(() => ({ ...defaultParams, ...params }), [params]);

  // Use useCallback to memoize the updateUniforms function
  const updateUniforms = useCallback(() => {
    if (!gl || !uniforms) return;
    gl.uniform1f(uniforms.u_edge, finalParams.edge);
    gl.uniform1f(uniforms.u_patternBlur, finalParams.patternBlur);
    gl.uniform1f(uniforms.u_time, 0);
    gl.uniform1f(uniforms.u_patternScale, finalParams.patternScale);
    gl.uniform1f(uniforms.u_refraction, finalParams.refraction);
    gl.uniform1f(uniforms.u_liquid, finalParams.liquid);
  }, [gl, uniforms, finalParams]);
  ...
  useEffect(() => {
    function initShader() {
      ...
      function getUniforms(program: WebGLProgram, gl: WebGL2RenderingContext): Record<string, WebGLUniformLocation> {
